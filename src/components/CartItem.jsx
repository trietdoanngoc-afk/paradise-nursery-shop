      ...
      {cartItems.length === 0 ? ( <p>Your cart is empty.</p> ) : (
        <div>
          {cartItems.map((item, index) => (
            <div key={index} className="cart-item-card" style={{display: 'flex', gap: '20px', marginBottom: '20px', alignItems: 'center'}}>
              <img src={item.image} alt={item.name} className="cart-item-img" style={{width: '120px', height: '120px', objectFit: 'cover'}} />
              <div>
                <h4>{item.name}</h4>
